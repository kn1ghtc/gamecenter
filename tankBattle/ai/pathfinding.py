...
class AdvancedPathfinding:
    ...
    def __init__(self, map_width: int = 800, map_height: int = 600, grid_size: int = 20):
        ...
        # 网格地图 - 用于快速查询墙体
        self.grid_map = np.zeros((self.grid_height, self.grid_width), dtype=np.int8)
    ...
    def build_grid_map(self, walls):
        """构建网格地图用于快速查询 - 一次性预处理"""
        self.grid_map.fill(0)
        self._cover_cache.clear()  # 清除旧的掩体缓存

        for wall in walls:
            if hasattr(wall, 'rect'):
                # 将墙体覆盖的所有网格标记为障碍物
                x_start = max(0, wall.rect.left // self.grid_size)
                x_end = min(self.grid_width - 1, wall.rect.right // self.grid_size)
                y_start = max(0, wall.rect.top // self.grid_size)
                y_end = min(self.grid_height - 1, wall.rect.bottom // self.grid_size)
    ...
    def find_tactical_path(self, start: Tuple[int, int], goal: Tuple[int, int],
        ...
        """寻找战术路径"""
        # 首次或墙体变化时重建网格地图
        if not hasattr(self, '_last_walls_count') or self._last_walls_count != len(walls):
            self.build_grid_map(walls)
            self._last_walls_count = len(walls)
    ...
    def _is_valid_position(self, grid_x: int, grid_y: int, walls, tank_size: Tuple[int, int]) -> bool:
        """检查网格位置是否有效 - 超优化版:使用网格地图O(1)查询"""
        if grid_x < 0 or grid_x >= self.grid_width or grid_y < 0 or grid_y >= self.grid_height:
            return False

        # 直接使用网格地图检查 - O(1)而不是O(n)
        # 检查坦克占据的所有网格
        tank_grid_w = (tank_size[0] + self.grid_size - 1) // self.grid_size
        tank_grid_h = (tank_size[1] + self.grid_size - 1) // self.grid_size

        for dy in range(tank_grid_h):
            for dx in range(tank_grid_w):
                check_x = grid_x + dx
                check_y = grid_y + dy
                if (check_x >= self.grid_width or check_y >= self.grid_height or
                    self.grid_map[check_y, check_x] > 0):
                    return False
    ...
    def _calculate_terrain_cost(self, grid_x: int, grid_y: int, walls) -> float:
        ...
        # 移除墙体类型检查 - 这个功能不重要但非常耗时(需要遍历所有墙体并计算距离)
        # 如果真的需要,应该预先构建墙体类型的网格地图
    ...
    def _calculate_cover_bonus(self, grid_x: int, grid_y: int, walls) -> float:
        """计算掩体奖励 - 优化版:使用网格缓存避免重复碰撞检测"""
        # 检查缓存
        cache_key = (grid_x, grid_y)
        if hasattr(self, '_cover_cache') and cache_key in self._cover_cache:
        ...
        # 检查四个主要方向的掩体 - 减少检查距离从3降到2
        directions = [(0, 1), (1, 0), (0, -1), (-1, 0)]

        for dx, dy in directions:
            for distance in range(1, 3):  # 只检查2个网格距离(原来3个)
                ...
                # 直接使用网格地图查询(O(1))而不是遍历所有墙壁(O(n))
                if self.grid_map[check_y, check_x] > 0:
        ...
        if not hasattr(self, '_cover_cache'):
            self._cover_cache = {}
        self._cover_cache[cache_key] = cover_score
